...
@Composable
fun PostItemGofit(title: String, date: String, user:String, place:String,accion: (String) -> Unit) {
    ElevatedCard(Modifier.fillMaxWidth().height(260.dp).padding(10.dp).combinedClickable(onClick = {accion(user)})) {
        Box(contentAlignment = Alignment.Center) {
            Column(Modifier.fillMaxSize()) {
                Row {
                    Image(
                        painter = painterResource(R.drawable.gofit),
                        contentDescription = "",
                    )
                }
                Row {
